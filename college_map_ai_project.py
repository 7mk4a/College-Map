...
def load_map_data(json_file_path="college_map_data.json"):
    """Loads node coordinates and connections from a JSON file."""
    ...
    try:
        ...
        print(f"Error: Map data file '{json_file_path}' not found. Please ensure it is in the same directory.")
        return False
    ...
    for node_name, data in map_data.items():
        # 1. Populate node_coordinates: {name: (x, y)}
        node_coordinates[node_name] = (data['x'], data['y'])
        
        # 2. Populate connections: {name: [neighbor1, neighbor2, ...]}
        # Note: The original code did not use the 'g(n)' value, so we only extract the neighbor name.
        connections[node_name] = [neighbor['name'] for neighbor in data['neighbors']]
        
    return True

# Load the data immediately
if not load_map_data():
    # If loading fails, the script will likely fail later, but we let it continue 
    # to allow the user to fix the JSON file.
    pass
...
# --- 4. A* Algorithm (Cumulative Time + Distance) ---
...
# --- 5. Execution Block ---
if __name__ == "__main__":            # Corrected from 'if name == "_main":'
    start_node = 'Point 2'
